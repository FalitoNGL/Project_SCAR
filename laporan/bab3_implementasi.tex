...
\begin{enumerate}[label=\arabic*.]
    ...
    \item \textbf{Intervensi \textit{Reverse Proxy}} (\textit{Middleboxes Buffering}): Menurut RFC~9110 (Fielding et al., 2022), perangkat perantara jaringan (seperti NGINX atau server \textit{cache}) seringkali melakukan \textit{buffering} penuh terhadap \textit{chunked response}. Bila honeypot ini diatur di belakang proxy semacam itu, \textit{garbage stream} abadi justru akan menghabiskan memori server proxy internal kita sendiri alih-alih merugikan infrastruktur penyerang.
